...
// Hash of the grammar file: 5C52A4792262DC6D523DC2ABAA974C86C56B10F9BC790AC09F8CBF146C1C5779
...
pub type SuffixOperator = String;
#[derive(Debug, Clone)]
pub struct DotCall {
...
mod peginator_generated {
    ...
    mod Suffix_impl {
        ...
        pub fn parse<'a>(
            ...
            ChoiceHelper::new(state)
                .choice(|state| {
                    parse_Whitespace(state, tracer, cache)
                        .and_then(|ParseOk { state, .. }| parse_SuffixOperator(state, tracer, cache))
                        .map_inner(Parsed__override::SuffixOperator)
                })
                .choice(|state| {
                    parse_Whitespace(state, tracer, cache)
    ...
    #[inline]
    pub(super) fn parse_SuffixOperator<'a>(
        state: ParseState<'a>,
        tracer: impl ParseTracer,
        cache: &mut ParseCache<'a>,
    ) -> ParseResult<'a, SuffixOperator> {
        tracer.run_traced("SuffixOperator", state, |state, tracer| SuffixOperator_impl::rule_parser(state, tracer, cache))
    }
    mod DotCall_impl {
        use super::*;
        #[inline(always)]
