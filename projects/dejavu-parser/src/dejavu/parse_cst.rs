...
fn parse_element(state: Input) -> Output {
    state.rule(NexusRule::Element, |s| {
        Err(s).or_else(|s|parse_text_elements(s).and_then(|s| s.tag_node("text_elements"))).or_else(|s|parse_template_export(s).and_then(|s| s.tag_node("template_export"))).or_else(|s|parse_template_if(s).and_then(|s| s.tag_node("template_if")))
    })
}
#[inline]
...
fn parse_template_if(state: Input) -> Output {
    state.rule(NexusRule::TemplateIf, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_if_begin(s).and_then(|s| s.tag_node("if_begin"))).and_then(|s|s.repeat(0..4294967295, |s|parse_if_else_if(s).and_then(|s| s.tag_node("if_else_if")))).and_then(|s|s.optional(|s|parse_if_else(s).and_then(|s| s.tag_node("if_else")))).and_then(|s|parse_if_end(s)))
    })
}
#[inline]
fn parse_if_begin(state: Input) -> Output {
    state.rule(NexusRule::IfBegin, |s| {
        s.sequence(|s|Ok(s).and_then(|s|s.sequence(|s|Ok(s).and_then(|s|parse_template_l(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_if(s)))).and_then(|s|builtin_ignore(s)).and_then(|s|s.sequence(|s|Ok(s).and_then(|s|s.sequence(|s|Ok(s).and_then(|s|parse_atomic(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_template_r(s)))).and_then(|s|s.repeat(0..4294967295, |s|parse_text_elements(s)).and_then(|s| s.tag_node("text")))).and_then(|s| s.tag_node("condition"))))
    })
}
#[inline]
fn parse_if_else(state: Input) -> Output {
    state.rule(NexusRule::IfElse, |s| {
        s.sequence(|s|Ok(s).and_then(|s|s.sequence(|s|Ok(s).and_then(|s|parse_template_l(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_else(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_template_r(s)))).and_then(|s|s.repeat(0..4294967295, |s|parse_text_elements(s)).and_then(|s| s.tag_node("text"))))
    })
}
#[inline]
fn parse_if_else_if(state: Input) -> Output {
    state.rule(NexusRule::IfElseIf, |s| {
        s.sequence(|s|Ok(s).and_then(|s|s.sequence(|s|Ok(s).and_then(|s|parse_template_l(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_else(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_if(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_template_r(s)))).and_then(|s|s.repeat(0..4294967295, |s|parse_text_elements(s)).and_then(|s| s.tag_node("text"))))
    })
}
#[inline]
fn parse_if_end(state: Input) -> Output {
    state.rule(NexusRule::IfEnd, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_template_l(s)).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_end(s)).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|parse_kw_if(s))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_template_r(s)))
    })
}
#[inline]
fn parse_kw_if(state: Input) -> Output {
    state.rule(NexusRule::KW_IF, |s| {
        s.match_string("if", false)
    })
}
#[inline]
fn parse_kw_else(state: Input) -> Output {
    state.rule(NexusRule::KW_ELSE, |s| {
...
fn parse_atomic(state: Input) -> Output {
    state.rule(NexusRule::Atomic, |s| {
        Err(s).or_else(|s|parse_boolean(s).and_then(|s| s.tag_node("boolean"))).or_else(|s|parse_identifier(s).and_then(|s| s.tag_node("identifier"))).or_else(|s|parse_number(s).and_then(|s| s.tag_node("number")))
    })
}
#[inline]
