...
fn parse_template_l(state: Input) -> Output {
    state.rule(DejavuRule::TEMPLATE_L, |s| {
        s.sequence(|s|Ok(s).and_then(|s|builtin_text(s, "<%", false)).and_then(|s|parse_space_control(s).and_then(|s| s.tag_node("space_control"))))
    })
}
#[inline]
fn parse_template_r(state: Input) -> Output {
    state.rule(DejavuRule::TEMPLATE_R, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_space_control(s).and_then(|s| s.tag_node("space_control"))).and_then(|s|builtin_text(s, "%>", false)))
    })
}
#[inline]
...
fn parse_if_begin(state: Input) -> Output {
    state.rule(DejavuRule::IfBegin, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_template_l(s).and_then(|s| s.tag_node("template_l"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_if(s).and_then(|s| s.tag_node("kw_if"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_atomic(s).and_then(|s| s.tag_node("atomic"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_template_r(s).and_then(|s| s.tag_node("template_r"))))
    })
}
#[inline]
fn parse_if_else(state: Input) -> Output {
    state.rule(DejavuRule::IfElse, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_template_l(s).and_then(|s| s.tag_node("template_l"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_else(s).and_then(|s| s.tag_node("kw_else"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_template_r(s).and_then(|s| s.tag_node("template_r"))))
    })
}
#[inline]
fn parse_if_else_if(state: Input) -> Output {
    state.rule(DejavuRule::IfElseIf, |s| {
        s.sequence(|s|Ok(s).and_then(|s|parse_template_l(s).and_then(|s| s.tag_node("template_l"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_else(s).and_then(|s| s.tag_node("kw_else"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_kw_if(s).and_then(|s| s.tag_node("kw_if"))).and_then(|s|builtin_ignore(s)).and_then(|s|parse_template_r(s).and_then(|s| s.tag_node("template_r"))))
    })
}
#[inline]
