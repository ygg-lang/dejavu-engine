...
// Hash of the grammar file: 22A7E9AC026C6C2B53B469B9401655714BA2F0B45CE8E879DE78C4874E61F369
...
pub type NumberNodeEx = String;
#[derive(Debug, Clone)]
pub struct UnicodeText {
    pub string: String,
    pub position: std::ops::Range<usize>,
}
pub type UnicodeTextEx = String;
#[derive(Debug, Clone)]
pub struct Comment {
    pub left: CommentL,
    pub right: CommentR,
...
mod peginator_generated {
    ...
    mod SlotIf_impl {
        ...
        mod part_2 {
            ...
            mod optional {
                ...
                pub fn parse<'a>(
                    ...
                    let ParseOk { result: r#else, state } = parse_Whitespace(state, tracer, cache)
                        .and_then(|ParseOk { state, .. }| parse_SlotElse(state, tracer, cache))
                        .map_inner(Some)?;
                    let ParseOk { result: empty, state } = parse_Whitespace(state, tracer, cache)
                        .and_then(|ParseOk { state, .. }| parse_SahaStatementNodes(state, tracer, cache))
                        .map_inner(Some)?;
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { result: start, state } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_SlotIfStart(state, tracer, cache))?;
            ...
            let ParseOk { result: end, state } =
                parse_Whitespace(state, tracer, cache).and_then(|ParseOk { state, .. }| parse_EndIf(state, tracer, cache))?;
    ...
    mod SlotEndIf_impl {
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { result: right, state } =
                parse_Whitespace(state, tracer, cache).and_then(|ParseOk { state, .. }| parse_SlotR(state, tracer, cache))?;
            Ok(ParseOk { result: Parsed { left, end, right }, state })
        }
        pub struct Parsed {
            pub left: SlotL,
            pub end: EndIf,
        ...
        pub fn rule_parser<'a>(
            ...
            let result = parse(state, tracer, cache)?.map(|r| super::SlotEndIf { left: r.left, end: r.end, right: r.right });
            Ok(result)
        }
    }
    #[inline]
    ...
    mod SlotFor_impl {
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { result: body, state } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_SahaStatementNodes(state, tracer, cache))?;
            let ParseOk { result: part_2::Parsed { r#else, empty }, state } = part_2::parse(state, tracer, cache)?;
            let ParseOk { result: end, state } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_SlotEndFor(state, tracer, cache))?;
            Ok(ParseOk { result: Parsed { start, body, r#else, empty, end }, state })
        }
        pub struct Parsed {
            pub start: SlotForStart,
    ...
    mod SlotEndFor_impl {
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { result: left, state } =
                parse_Whitespace(state, tracer, cache).and_then(|ParseOk { state, .. }| parse_SlotL(state, tracer, cache))?;
            let ParseOk { result: end, state } =
                parse_Whitespace(state, tracer, cache).and_then(|ParseOk { state, .. }| parse_EndFor(state, tracer, cache))?;
            let ParseOk { result: right, state } =
                parse_Whitespace(state, tracer, cache).and_then(|ParseOk { state, .. }| parse_SlotR(state, tracer, cache))?;
            Ok(ParseOk { result: Parsed { left, end, right }, state })
        }
        ...
        pub fn rule_parser<'a>(
            ...
            let result = parse(state, tracer, cache)?.map(|r| super::SlotEndFor { left: r.left, end: r.end, right: r.right });
            Ok(result)
        }
    }
    #[inline]
    ...
    mod NumberNode_impl {
        ...
        pub fn parse<'a>(
            ...
            parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_NumberNodeEx(state, tracer, cache))
                .discard_result()
        }
        pub type Parsed = ();
        #[inline(always)]
    ...
    #[inline]
    pub(super) fn parse_NumberNodeEx<'a>(
        state: ParseState<'a>,
        tracer: impl ParseTracer,
        cache: &mut ParseCache<'a>,
    ) -> ParseResult<'a, NumberNodeEx> {
        match crate::utils::maybe_number(state.s()) {
            ...
            Err(error_string) => Err(state.report_error(ParseErrorSpecifics::ExternRuleFailed {
                function_name: "crate::utils::maybe_number",
                error_string,
            })),
        }
    }
    mod UnicodeText_impl {
        ...
        pub fn parse<'a>(
            ...
            parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_UnicodeTextEx(state, tracer, cache))
                .discard_result()
        }
        pub type Parsed = ();
        #[inline(always)]
    ...
    #[inline]
    pub(super) fn parse_UnicodeTextEx<'a>(
        state: ParseState<'a>,
        tracer: impl ParseTracer,
        cache: &mut ParseCache<'a>,
    ) -> ParseResult<'a, UnicodeTextEx> {
