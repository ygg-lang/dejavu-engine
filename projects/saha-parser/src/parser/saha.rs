...
// Hash of the grammar file: 61DCA08CDD80BAB338A2CD7AAA6BDAA0DD1153A15149836251F6488F0297DC02
    ...
    mod SlotIf_impl {
        ...
        pub fn parse<'a>(
            ...
            let ParseOk { result: end, state } = parse_Whitespace(state, tracer, cache)
                .and_then(|ParseOk { state, .. }| parse_SlotEndIf(state, tracer, cache))?;
        ...
        mod choice_0 {
            ...
            pub fn parse<'a>(
                ...
                let ParseOk { state, .. } = parse_Whitespace(state, tracer, cache)
                    ...
                    .discard_result()?;
                let ParseOk { state, .. } = part_2::parse(state, tracer, cache)?;
                let ParseOk { state, .. } = part_3::parse(state, tracer, cache)?;
                Ok(ParseOk { result: (), state })
            }
            pub type Parsed = ();
        }
